...
class EmbedDetector(BaseDetector):
    def __init__(self, opt):
        super(EmbedDetector, self).__init__(opt)
        self.opt = opt
        ...
        self.corre_mat = torch.tensor(self.corre_mat.transpose()).float().cuda()

    def process(self, images, return_time=False):

        with torch.no_grad():
            output = self.model.inference(images)[-1]
            torch.cuda.synchronize()
            forward_time = time.time()

            dets_obj, dets_sub, rel = self.model.decode(output, corremat=self.corre_mat, is_sub_verb=self.opt.use_verb_sub)
