...
class HICOEmbd(HICO):
    ...
    def __getitem__(self, index):
        ...
        sub_ind = np.zeros((self.max_rels), dtype=np.int64)
        obj_ind = np.zeros((self.max_rels), dtype=np.int64)
        rel_mask = np.zeros((self.max_rels), dtype=np.uint8)
        rel_cls = np.zeros((self.max_rels, self.num_classes_verb), dtype=np.float32)
        ...
        for k in range(num_rels):
            ...
            obj = hoi['object_id']
            hoi_cate = int(self.cat_ids_verb[hoi['category_id']])
            rel_dict.setdefault((sub, obj), []).append(hoi_cate)
        
        for i, k in enumerate(rel_dict.keys()):
            sub_ind[i] = k[0]
            obj_ind[i] = k[1]
            rel_mask[i] = 1
            for cate in rel_dict[k]:
                rel_cls[i, cate] = 1.

        ret = {'input': inp, 'hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'wh': wh,
               'sub_ind': sub_ind, 'obj_ind': obj_ind, 'rel_mask': rel_mask, 'rel_cls': rel_cls}
